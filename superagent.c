...
int getFirstNoneAcked(int *mem, int win_size, int win_start_idx) {
    int i;
    int min_noneacked_idx = MAX_WINDOW;
    for(i = 0; i < win_size; i++) {
        if(mem[i + win_start_idx] == 0) {
            min_noneacked_idx = min_noneacked_idx < i + win_start_idx ? min_noneacked_idx : i + win_start_idx;
        }
    }
    if(min_noneacked_idx == MAX_WINDOW) {
	return -1;
    }
    return min_noneacked_idx;
}
